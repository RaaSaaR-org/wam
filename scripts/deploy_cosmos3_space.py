#!/usr/bin/env python3
"""Deploy the Cosmos3-Nano readout probe as a ZeroGPU Space (free GPU on a PRO account).

Sibling of `deploy_wan_space.py` (T-24 backbone bake-off). Uploads
`scripts/hf_job_cosmos3_probe.py` verbatim as the Space's `cosmos_probe.py`, plus the Wan
probe + smoke + converter scripts it imports — one implementation of windows, labels and
ridge machinery for both backbones, no vendored copy to drift.

    python scripts/deploy_cosmos3_space.py --dry-run
    python scripts/deploy_cosmos3_space.py              # create/update, then open the URL
...
SPACE_DIR = REPO_ROOT / "deploy" / "cosmos3-probe-space"
COSMOS_SCRIPT = REPO_ROOT / "scripts" / "hf_job_cosmos3_probe.py"
...
DEFAULT_SPACE = "wam-cosmos3-probe"
...
def pinned_requirements() -> tuple[str, str]:
    ...
    previous code. The SHA changes the key, and it records exactly which commit was
    tested (AC-04).
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    api.create_commit(
        ...
        commit_message=f"deploy WAM Cosmos3 readout probe @ {sha[:8]}",
    )
