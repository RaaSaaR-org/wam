...
def area_bound_missing_message(path: pathlib.Path, why: str) -> str:
    ...
    return "\n".join([
        ...
        "",
        "       WHY THIS FILE WILL NOT PICK ONE. The obvious derived bound is useless: the maximum",
        "       observed over the source corpus can never be exceeded by a composite that runs on",
        "       those same frames, so a bound that CAN fire sits above it by a margin, and that",
        "       margin is coined however it is dressed. So the measurement is automated and the",
        "       decision is not.",
        "",
        "       WHAT HAS ALREADY BEEN DONE, so that this refusal is not read as 'start from",
        "       nothing'. This paragraph said 'the distribution HAS NEVER BEEN MEASURED' until",
        "       2026-08-26 and that is no longer true; the sentence was retired by the measurement",
        "       rather than edited around it. The distribution IS measured — 402 episodes, 171 625",
        "       frames, stride 1, measurement_qualified true, pooled in",
        "       runs/pr08-robot-mask-area/POOLED.json — and a bound WAS decided on it under",
        "       T40_RULE_V13: max_frame_fraction 0.6409114583333333 — written into the artifact and",
        "       the rule as 0.64091145833333329, which is the same double and prints shorter — the",
        "       midpoint of a gap containing zero of those frames, with the rationale in the",
        "       artifact beside it. So if you are",
        "       reading this refusal, the likely cause is not that the work is undone but that this",
        "       tree does not carry the committed artifact, or that it carries a DIFFERENT one — a",
        "       different segmenter, a different prompt, or a different source manifest. Read the",
        "       first line: it names which.",
        "",
        "       WHAT TO DO IF IT IS GENUINELY UNMEASURED HERE.",
