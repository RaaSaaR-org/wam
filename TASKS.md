...
- [ ] **[T-040](.mc/tasks/todo/T-040-cosmos-transfer-photoreal-augmentation.md)** Cosmos-Transfer2.5
  photoreal augmentation — pre-register before generating anything *(backlog, blocked on T-39; the
  deliverable is `PR-08` + the T-34 screen, not a corpus. Consumer would be `emai/vla-training`.
  Collides with "sim frames are NOT training data" (T-25) and with PR-06's measured result that
  generated video predicts worse than a frozen frame)*
- [ ] **[T-041](.mc/tasks/todo/T-041-cosmos-generator-finetune-on-g1.md)** Fine-tune a Cosmos
  generator on G1 data *(backlog, **frozen** — PR-07 §7 names Cosmos3-Super generation in its
  freeze list. Scoped so the idea carries its costs; the better fix for the embodiment defect, and
  a much larger project than T-040)*
