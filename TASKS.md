...
- [ ] **[T-041](.mc/tasks/todo/T-041-cosmos-generator-finetune-on-g1.md)** Fine-tune a Cosmos
  ...
  **plus `Humanoid-Everyday-G1`, added 2026-08-07 by operator decision, which is what engages
  OD-09**; PR-09 §2 carries the dated amendment and what it costs. The captioner is NVIDIA's
  shipped `caption_from_video`, not ours to build.
  **Unblocked by OD-10** — PR-07 §7's Cosmos3-Super clause lifted, T-32 and Cosmos3-Edge still
  ...
  staged (jobs 186281/186282, COMPLETED); corpus fetch + captioning queued (186332/186333). The
  probe is the next gate and needs a human to read `PROBE.json`; job 95 additionally needs 20
  calibration clips nobody has picked yet. **Open before the run:** the corpus is 640×480 4:3
  throughout while §5 generates 720p 16:9, and 500 iterations is under one epoch now that the
  corpus has roughly quadrupled)*
