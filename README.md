...
| **That any method at all clears this bar on this corpus** | Fourteen experiments, fourteen negatives, and every one compares a WAM variant against a *trivial* baseline. The corpus is `nvidia/GR00T-N1.7-AppleToPlate`, NVIDIA's own tutorial dataset for post-training GR00T on this exact task, whose recipe appears nowhere in this repo, so "not enough data" was never distinguishable from "we have not run a method that works on this much data" (`docs/preregistration/PR-07-positive-control.md`). **Corrected 2026-08-16 — T-39 HAS run.** It returned `VOID (labels)`, and PR-12 (`C`) then PR-13 (`W`) traced that VOID to **our own evaluation adapter, not the corpus**: the chunk's step 0 differenced a command against a *state* while every other step differences command against command, and that one term carried ~90 % of the error. Repaired, the corpus's own action column scores **+68.10 L1 / +75.40 L2, level L4** on T-39's own holdout. **Still true and unchanged: the fourteen negatives above stand** (they were scored WAM-format against WAM-format, and `src/wam/` never contained the defect), and **no policy has been trained on these labels** — every number here scores oracles against oracles. Whether that training run happens is the project owner's call. |
